package br.felipefcosta.mobchat.utils

object Constants {
    const val TOKEN_URL = "https://mobchat-iammicroservice-identity.azurewebsites.net/"
    const val APPUSER_API_URL = "https://mobchat-iammicroservice-api.azurewebsites.net/api/"
    const val PROFILE_API_URL = "https://mobchat-profilemicroservice-api.azurewebsites.net/api/"
    const val CHATHUB_API_URL = "https://mobchat-chatmicroservice.azurewebsites.net/hubs/chathub"
    const val BLOB_STORAGE_CONNECTION_STRING = "DefaultEndpointsProtocol=https;AccountName=" +
            "mobchatstoragecontainer;AccountKey=oBH56HdLB5s1ej3fNEB8MCPbdA4VdzPi/XaW364M6XS0PP1+" +
            "q28uDuKFHmniReFELzKfhfzE2/5PO96kAUVztA==;EndpointSuffix=core.windows.net";
    const val GRANT_TYPE = "password"
    const val EMAIL = "admin"
    const val PASSWORD = "00ProDev61#MobChat00"
    const val CLIENT_ID = "MobChatUserMobileApp_ClientId"

}

